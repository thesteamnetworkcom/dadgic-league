...
import type { CreateLeagueRequest } from '@dadgic/shared'
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!userId) {
      return NextResponse.json({
        success: false,
        error: 'User context required for league creation',
        details: [{ field: 'context.user_id', message: 'User ID required for admin validation' }],
        timestamp: new Date().toISOString()
      }, { status: 400 })
    }

    // 3. CALL SERVICE (follow pod pattern)
    const result = await createLeague(body, userId)
